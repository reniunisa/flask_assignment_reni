...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(['Category', 'App']).agg({'Reviews': 'sum', 'Rating': 'mean'}).reset_index().sort_values(by='Reviews', ascending=False).head(10).reset_index(drop=True).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
